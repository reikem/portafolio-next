...
export const AssassinModal = forwardRef<HTMLDivElement, AssassinModalProps>(
    ({ show, currentRiddle, riddleAnswer, showResult, isCorrect, onAnswerChange, onSubmit, onClose }, ref) => {
      const { t } = useLanguage()
        ...
        >
          ...
          >
            <div className="text-center mb-6">
              <h2 className="text-4xl font-bold text-white mb-2">⚔️ {t.assassinTitle} ⚔️</h2>
              <p className="text-red-400 italic">{t.assassinSubtitle}</p>
            </div>
              ...
              <div className="space-y-6">
                {isCorrect ? (
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-white italic">{Math.random() > 0.5 ? currentRiddle.altair : currentRiddle.ezio}</p>
                  </div>
                ) : (
                  <div className="text-center space-y-4">
                    <div className="text-6xl">⚔️</div>
                    <p className="text-red-400 text-xl font-bold">{t.incorrect}</p>
                  </div>
                )}
                <button
                  onClick={onClose}
